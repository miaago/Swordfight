...
public class PlayerMovement : MonoBehaviour
{
    ...
    // Variables the AI accesses since it will use the same behavior as the player
    [HideInInspector] public float externalH, externalV;
    ...
    private void FixedUpdate()
    {
        if (isAI)
        {
            Debug.Log($"AI Physics Check: State: {state} | MoveSpeed: {moveSpeed} | Grounded: {isGrounded}");
        }
        MovePlayer();
    }
    ...
    private void MyInput()
    {
        ...
        else if (isAI) // AI logic
        {
            horizontalInput = externalH;
            verticalInput = externalV;

            if (externalJump && canJump && isGrounded)
            {
                PerformJump();
            }
        }
    ...
    // Seperate function so both player and enemy can call
    ...
    private void StateHandler()
    {
        ...
        else if (isGrounded && (Input.GetKey(sprintKey) || (isAI && isSprinting))) // Sprinting
        {
            state = MovementState.sprinting;
            desiredMoveSpeed = sprintSpeed;
            if (cam != null) cam.DoFov(sprintFov);
        }
        else if (isGrounded) // Walking
        {
            state = MovementState.walking;
    ...
    private void MovePlayer()
    {
        ...
        if (isAI)
        {
            float currentForceMagnitude = moveSpeed * 10f;
            // This will tell us if moveSpeed is 0 or if the inputs are 0
            if (moveDirection.magnitude < 0.1f)
                Debug.LogWarning($"AI has NO MoveDirection. V: {verticalInput} H: {horizontalInput}");

            if (moveSpeed < 1f)
                Debug.LogWarning($"AI MoveSpeed is too low: {moveSpeed}");
        }
        // --- DEBUG LOGS END ---

        if (OnSlope() && !isExitingSlope)
        {
            if (isAI) Debug.Log("AI Status: Moving on Slope");
            rb.AddForce(GetSlopeMoveDirection() * moveSpeed * 20f, ForceMode.Force);
        ...
        {
            if (isAI) Debug.Log("AI Status: Moving on Ground");
            rb.AddForce(moveDirection * moveSpeed * 10f, ForceMode.Force);
        }
        else if (!isGrounded)
        {
            if (isAI) Debug.Log("AI Status: In Air");
            rb.AddForce(moveDirection * moveSpeed * airMultiplier * 10f, ForceMode.Force);
        }
